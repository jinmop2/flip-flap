...
// 늘었는지 줄었는지 알 수 있다.
// ── 더 세게 만들려다 안 된 것들 (2026-09) ────────────────────────────────
//
// 아래 다섯은 재 보고 접었다. 안 적어 두면 다음 사람이 같은 곳을 또 판다.
// 전부 상대를 여럿 놓고 5000~15000판씩 잰 결과다 (tools/sweep.js).
//
//   ① 중반도 몇 수 굴려 본다        깊이 1·2·3·5 → 전부 ±2%p 안. 소득 없음.
//      깊이 0(경매 하나만 정산하고 진행도로 판정)은 -15.4%p 로 크게 나빴다.
//      진행도 잣대(endScore)는 세트가 안 된 자리에서 0.85/0.05 둘로만 갈려
//      한 걸음 차이를 못 본다 — 얕게 쓰면 안 되는 자다.
//   ② 롤아웃의 경매 방식 바로잡기    롤아웃은 미래를 전부 클로즈로 그리는데
//      실제로는 열에 아홉이 오픈이다. 0/0.1/0.3/1.0 로 바꿔도 전부 같았다.
//      (틀린 건 맞지만 승률은 안 움직인다.)
//   ③ 상상 속 상대에게도 캐치업 보정  같았다.
//   ④ 생각량 늘리기                 표본 240·400, 종반 깊이 9·11 — 같거나 낮다.
//      지금의 160/7 이 이미 꼭대기다.
//   ⑤ 출품도 배팅과 같은 잣대로       표본 120·300 — 같았다. 지금 것을 그대로 둔다.
//
// 움직인 것은 아래 COST_POW 하나뿐이다. 이 게임의 AI 실력은 더 깊이 읽어서가
// 아니라 무엇을 값지게 보느냐로 갈린다.
...
// ── 강카드를 넘기는 손해의 무게 ──────────────────────────────────────────
//
// 배팅 카드는 이긴 쪽이 가져가는 게 아니라 서로 교환된다. 그래서 강한 카드를
// 부르는 값에는 "이 카드가 상대 손으로 간다" 는 손해가 들어 있다. v3 는 그
// 무게를 0.5 로 손으로 넣었고, 그 뒤로 한 번도 재 본 적이 없었다.
//
// 상대 셋(옛 전문가·v3·보통AI)을 상대당 20000판씩 (tools/sweep.js):
//                     옛 전문가   v3 상대   보통 상대
//   0.5 (예전 값)        49.4%     51.9%     85.7%   ← 눈금 확인용 대조군
//   0.7 (지금 값)        51.2%     52.0%     86.0%
//   0.8                  52.0%     52.6%     85.0%
//
// 숫자만 보면 0.8 이 낫다. 그런데 0.8 은 값진 경매(가치 0.79)에서도 최약
// 카드를 던지고 물러난다 — 400번 중 400번. 0.7 까지는 0번이다. 절벽이 그
// 사이에 있다.
//
// 그렇게 되면 아이템전의 "덤이 걸린 판만 일부러 져 준다" 가 통째로 죽는다.
// 늘 져 주는 AI 는 덤을 봤다고 더 져 줄 수가 없다. 사람 눈에도 그냥 무른
// AI 로 보인다. 이겨야 할 판에서 무르게 두는 것은 승률표에 안 잡히는 손해다.
// tests/t_ai4.js 의 '덤이 없으면 경매를 다툰다' 가 이 선을 지킨다.
//
// 상대를 여럿 놓는 것도 중요하다. 옛 전문가 하나만 놓고 골랐을 때는 다른 값
// (경매를 이기는 값 tempo 를 -0.20 으로)이 +4%p 로 보였는데, 상대를 늘리자
// 약한 상대에게 5%p 씩 밀렸다 — 그 상대만 잡는 수였다.
const COST_POW = process.env.FF_COSTPOW ? +process.env.FF_COSTPOW : 0.7;

// 롤아웃 안의 앞으로의 경매를 얼마나 클로즈로 볼 것인가. 예전엔 늘 클로즈였는데
// typeV3 는 열에 아홉을 오픈으로 고른다 — 두지도 않을 판을 그리고 있었다.
// 고쳐도 승률은 안 움직였지만(0/0.1/0.3/1.0 전부 같음), 틀린 것을 맞게 둔다.
const CLOSED_RATE = process.env.FF_CLOSEDRATE ? +process.env.FF_CLOSEDRATE : 0.10;

function bidV4(view, mem) {
  return X3.bidV3({ ...view, samples: SAMPLES, endgameDepth: EG_DEPTH,
                    costPow: COST_POW, closedRate: CLOSED_RATE }, mem);
}
...
module.exports = {
  ...
  // tools/duel.js 는 v3 의 이름으로 부른다 — 같은 저울에 올리려면 이름을 맞춘다
  bidV3: bidV4, offerV3: offerV4, typeV3: X3.typeV3,
};
