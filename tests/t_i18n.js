...
console.log('\n⑥ 사람이 쓴 글은 건드리지 않는다');
{
  ...
  // 여기서 번역이 걸리면 남의 이름을 멋대로 바꾸는 셈이 된다.
  ok('모르는 닉네임은 그대로', FF.t('훑기시험') === '훑기시험');
  ok('모르는 클랜 이름도 그대로', FF.t('수다클랜42981') === '수다클랜42981');
  // 값이 끼는 문구에서도 사람 이름은 살아남아야 한다
  ok('문장 속 닉네임도 그대로', FF.t('\u26a1 훑기시험 선공!') === '\u26a1 훑기시험 goes first!',
     FF.t('\u26a1 훑기시험 선공!'));
}
